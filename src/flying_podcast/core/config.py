...
class Settings:
    ...
    target_article_count: int = _env_int("TARGET_ARTICLE_COUNT", 8)
    domestic_ratio: float = _env_float("DOMESTIC_RATIO", 0.0)
    min_tier_a_ratio: float = _env_float("MIN_TIER_A_RATIO", 0.3)
    ...
    max_entries_per_source: int = _env_int("MAX_ENTRIES_PER_SOURCE", 4)
