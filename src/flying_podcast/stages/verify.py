...
def _llm_editor_review(
    ...
    system_prompt = (
        ...
        "这些文章已经过AI选稿、评分和翻译，只有评分达标的才会到你这里。\n"
        "你的职责是把关内容质量，找出有明显问题的文章。追求质量，不求数量。\n\n"
        "【审查维度】请从以下四个方面逐条审查：\n"
        "1. 内容价值：文章是否包含至少一个具体的新闻事实（事件、数据、政策等）？\n"
        "   删除：完全没有具体事实的纯概述/科普/表态/口号文章。\n"
        "   保留：只要有具体事件、具体数据、具体时间地点，就有发布价值。\n"
        "2. 翻译/写作质量：中文是否通顺可读？\n"
        "   删除：严重机翻痕迹导致读不懂、关键信息表述混乱的文章。\n"
        "   保留：表述基本通顺、要点清晰可理解的文章（不要求完美文笔）。\n"
        "3. 事实与逻辑：标题与正文是否一致？内容是否自相矛盾？\n"
        "   删除：标题与正文严重不符、要点之间明显矛盾的文章。\n"
        "4. 重复：与本期其他文章是否报道同一核心事件？\n"
        "   删除：与同期另一篇实质重复的（保留质量更好的那篇）。\n\n"
        "【重要原则】\n"
        "- 有具体事实的文章应该保留，即使事实的运行参考价值有限。\n"
        "- 只删除有明显硬伤的文章，不要因为'还不够好'就删。\n"
        "- 如果拿不准，保留。\n\n"
        "【输出要求】\n"
