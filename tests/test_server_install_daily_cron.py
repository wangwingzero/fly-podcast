...
def test_build_crontab_replaces_legacy_schedules():
    ...
    assert "# flying-podcast daily digest: Beijing 03:00, auto-publishes non-empty WeChat digest" in updated
    assert "0 3 * * *  flock -xn /www/server/cron/flying_podcast_daily.lock" in updated
